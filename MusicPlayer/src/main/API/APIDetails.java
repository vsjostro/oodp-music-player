...
public class APIDetails {
    ...
    public String getAlbumArtPath(String songName, String artistName, String bigAlbumArtURL) throws IOException {
        ...
        String parsedSongName = songName.replaceAll("[^a-zA-Z0-9]+", "");
        String parsedArtistName = artistName.replaceAll("[^a-zA-Z0-9]+", "");

        String fileName = parsedSongName + parsedArtistName + ".jpg";
    ...
    private String extractLyrics(String songName, String artistName, String clientAccessToken) throws Exception {
        ...
        String fileName = parsedSongName + parsedArtistName + ".txt";
        fileName = fileName.replace(' ', '_');
        fileName = fileName.replace("\"", "");
        ...
        try {
            ...
            if (os.contains("mac")) {
                ...
                String execPath = URLDecoder.decode(path, "UTF-8");
                if (execPath.contains("MusicPlayer.jar")) {
                    execPath = execPath.replace("MusicPlayer.jar", "classes/main/API/dist/lyricParser");
                    System.out.println(execPath);
                } else {
                    execPath = loader.getResource("main/API/dist/lyricParser").getPath();
                }
                ...
                Process process = Runtime.getRuntime().exec(new String[]{
                        exePath, songName, artistName, clientAccessToken});
                process.waitFor();
